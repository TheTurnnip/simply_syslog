...
class Server:
    ...
    def __init__(self, config: Config, buffer: NetworkBuffer) -> None:
        """
        Initializes an instance of the Server class.

        Args:
            config (config.Config): The configuration for the server.
        ...
        Notes:
            The stop method has not yet been implemented.
        """
        ...
        self.logger = setup_server_logging(config.do_debug_logging)
    ...
    def make_udp_server(self) -> socket.socket:
        ...
        address_and_port = (self.config.bind_address, self.config.udp_port)
        udp_server.bind(address_and_port)
        self.logger.info(f"Created UDP socket on port {self.config.udp_port}.")
        return udp_server
    ...
    def make_tcp_server(self) -> socket.socket:
        ...
        address_and_port = (self.config.bind_address, self.config.tcp_port)
        tcp_server.bind(address_and_port)
        tcp_server.listen(self.config.max_tcp_connections)
        self.logger.info(f"Created TCP socket and started listening on port "
                         f"{self.config.tcp_port}.")
        return tcp_server
    ...
    def start(self) -> None:
        ...
        """
        match self.config.protocol.upper().strip():
            case "UDP":
